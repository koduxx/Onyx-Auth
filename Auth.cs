...
namespace onyx.LOGIN
{
  internal class Auth
  {
    public static string GetDevicecodetoken()
    {
      ...
      try
      {
        str = strArray[1].ToString().Split(new char[1]
        {
          ','
    ...
    public static string GetToken(string authCode)
    {
      ...
      {
        int num = (int) MessageBox.Show("An Error occured, pls try again at a later point.");
      ...
      int num1 = (int) MessageBox.Show(content);
      return "error";
    }
